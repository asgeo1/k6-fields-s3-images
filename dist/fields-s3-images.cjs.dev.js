...
var utils = require('./utils-0512e9c3.cjs.dev.js');
...
async function getDataFromStream(config, upload, context) {
  ...
  const s3Client = new clientS3.S3Client(config.s3Options);
  ...
  const fullUpload = new libStorage.Upload({
    client: s3Client,
  ...
  if (sm) {
    ...
    const smUpload = new libStorage.Upload({
      client: s3Client,
      params: _objectSpread({
        Body: smFile.data,
        ContentType: mimetype,
  ...
  if (md) {
    ...
    const mdUpload = new libStorage.Upload({
      client: s3Client,
      params: _objectSpread({
        Body: mdFile.data,
        ContentType: mimetype,
  ...
  if (lg) {
    ...
    const lgUpload = new libStorage.Upload({
      client: s3Client,
      params: _objectSpread({
        Body: lgFile.data,
        ContentType: mimetype,
